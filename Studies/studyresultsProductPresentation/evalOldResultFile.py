...
fn = "results_2023-06-19_22:56:32.json"  # run used for product presentation
...
if resultsType == "3d":
    ...
    for idx, rob_id in enumerate(robots_ids):
        for resolution in resolutions:
            for sample_size in sample_sizes:
                for slc in slices:
                    dp = results["data"][rob_id][resolution][str(slc)]["fk"][sample_size]
    ...
    for res in resolutions:
        for s_cnt in sample_sizes:
            ...
            avg_time = round(total_time / len(robots_ids) / len(slices), 2)
            print("---------------------------")
